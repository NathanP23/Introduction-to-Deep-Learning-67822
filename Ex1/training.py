...
def setup_training(model, y_train):
    ...
    loss_fn = nn.BCELoss(pos_weight=pos_weight)  # Binary Cross Entropy Loss (since sigmoid is in model)
...
def train_model(model, train_loader, test_loader, loss_fn, optimizer, epochs=EPOCHS):
    ...
    for epoch in range(epochs):
        ...
        epoch_test_loss = test_loss / len(test_loader.dataset)
        epoch_accuracy = 100 * correct / len(test_loader.dataset)
        test_losses.append(epoch_test_loss)
        accuracies.append(epoch_accuracy)

        print(f"Epoch {epoch+1}/{epochs} | Train Loss: {epoch_train_loss:.4f} | "
              f"Test Loss: {epoch_test_loss:.4f} | Accuracy: {epoch_accuracy:.2f}%")
