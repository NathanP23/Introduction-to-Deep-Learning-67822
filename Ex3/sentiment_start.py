...
# CODE_CHANGE: Initialize logs for visualization
################################################
train_loss_log = []
test_loss_log = []
test_step_log = []
...
for epoch in range(num_epochs):
    ...
    for labels, reviews, reviews_text in train_dataset:   # getting training batches
        ...
        # averaged losses
        # CODE_CHANGE: Log losses for visualization
        ################################################
        loss_value = float(loss.detach())

        if test_iter:
            test_loss = 0.8 * loss_value + 0.2 * test_loss
            test_loss_log.append(test_loss)
            test_step_log.append(len(train_loss_log))  # Align with global step index
        else:
            train_loss = 0.9 * loss_value + 0.1 * train_loss
...
# CODE_CHANGE: Visualize training and test loss
################################################
viz.plot_loss_curves(train_loss_log, test_loss_log, test_step_log)
################################################